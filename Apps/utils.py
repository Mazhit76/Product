...
LOG = logging.getLogger('app_utils.main')
...
class ClientServer:
    ...
    def serializer_to_byte(self, message, CONFIG) -> bytes:
        if isinstance(message, dict):
            try:
                ...
                LOG.error(f'Ошибка преобразования файла в json. Возможно тип кодировки непраильный. message: {message} , кодировка {CONFIG.get("ENCODING")}')
        ...
        if isinstance(byte_str, bytes):
            try:
                ...
                try:
                    ...
                    LOG.error(f'Ошибка. На выходе преобразования json, находится не словарь. {byte_str} , кодировка {CONFIG.get("ENCODING")}')
                    raise ValueError('Ошибка. На выходе преобразования json, находится не словарь.')

            except (JSONDecodeError, LookupError):
                LOG.error(f'Ошибка преобразования файла из json в словарь. Возможно тип кодировки неправильный. {byte_str} , кодировка {CONFIG.get("ENCODING")}')
                raise ValueError('Ошибка преобразования файла из json в словарь. Возможно тип кодировки неправильный.')
        else:
            LOG.error(f'Ошибка. На вход подан не байтовая строк для преобразования в словарь. {byte_str} , кодировка {CONFIG.get("ENCODING")}')
    ...
    def get_message(self, opened_socket, CONFIG):
        if isinstance(CONFIG.get('MAX_PACKAGE_LENGTH'), int):
            ...
            LOG.error(f'Размер данных для сокете слишком большой или отрицательный {CONFIG.get("MAX_PACKAGE_LENGTH")} socket: {opened_socket}')
            raise ValueError('Размер данных для сокете слишком большой или отрицательный')
        LOG.error(f'Размер данных для сокете не в виде числа {CONFIG.get("MAX_PACKAGE_LENGTH")} socket: {opened_socket}')
